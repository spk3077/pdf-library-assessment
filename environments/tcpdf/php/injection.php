...
foreach ($escape_seq as $seq) {
    $pdf = createPDF();
    $pdf->Image('@'.$imgdata.$seq, 15, 140, 50, 50, '', '', '', true, 150, '', false, false, 1, false, false, false);
    $pdf->Text(20, 20, "DOGTEST");
...
foreach ($escape_seq as $seq) {
    $pdf = createPDF();
    $pdf->Image("/var/www/myapp/images/xref.jpg", 15, 140, 50, 50, "JPEG", $seq, '', true, 150, '', false, false, 1, false, false, false);
    $pdf->Text(20, 20, "DOGTEST");
...
foreach ($escape_seq as $seq) {
    $pdf = createPDF();
    $pdf->Write(0, $seq, $seq, 0, 'C', true, 0, false, false, 0);
    $pdf->Text(20, 20, "DOGTEST");
...
foreach ($escape_seq as $seq) {
    $pdf = createPDF();
    $pdf->Cell(0, 0, $seq, 1, 1, 'C', 0, $seq, 0);
    $pdf->Text(20, 20, "DOGTEST");
...
foreach ($escape_seq as $seq) {
    $pdf = createPDF();
    $pdf->Annotation(40, 40, 10, 10, $seq, array('Subtype'=>'Text', 'Name' => 'Comment', 'T' => $seq, 'Subj' => $seq, 'C' => array(255, 255, 0)));
    $pdf->Text(20, 20, "DOGTEST");
